...
public class Player : MonoBehaviour
{
    ...
    private SpriteRenderer guaRenderer;
    ...
    private bool isHelpInvinsable = false;
    private bool isHelpThunder = false;
    private int helpDamageRate = 1;
        ...
        for (int i = 0; i <= guaDict["Qian"]; i++)
        {
            for (int j = 0; j <= guaDict["Kun"]; j++)
            {
                ...
                if (guaDict["Li"] > 0 && guaDict["Kan"] > 0)
                {
                    ...
                    projectile1.damage = helpDamageRate;
                }
                ...
                if (j > 0)
                {
                    if (guaDict["Li"] > 0 && guaDict["Kan"] > 0)
                    {
                        ...
                        projectile2.damage = helpDamageRate;
                    }
                    else if (guaDict["Li"] > 0)
                    {
    ...
    public void GetYao(bool isYang)
    {
        ...
        if (yaoList.Count >= 3)
        {
            var effect = Instantiate(guaEffect, new Vector3(0f, 2.38f, 0f), Quaternion.identity);
            guaRenderer = effect.GetComponent<SpriteRenderer>();
            guaRenderer.sprite = guas[(yaoList[0] ? 1 : 0)*4 + (yaoList[1] ? 1 : 0)*2 + (yaoList[2] ? 1 : 0)];
    ...
    {   
        ...
        if ((invincibleTimer > invincibleTime) && !isHelpInvinsable)
        {
            if (shield > 0)
            {
                shield -= 1;
    ...
    public void SetHelpInvinsable(bool activate)
    {
        isHelpInvinsable = activate;
    }

    public void SetHelpThunder(bool activate)
    {
        isHelpThunder = activate;
        if (activate)
        {
            zhenCooldownTime = 0f;
        }
    ...
    public void SetHelpQiNumber(int number)
    {
        MAX_QI = number;
    }

    public void SetHelpDamage(int damage)
    {
        helpDamageRate = damage;
    }
    ...
    private void SaveByPlayerPrefs()
    {
        ...
        saveData.waveNum_save = waveManager.waveNum;
    ...
    private void LoadFromPlayerPrefs()
    {
        ...
        defaultSaveData.waveNum_save = 0;
        defaultSaveData.yaoList_save = new List<bool>();
        defaultSaveData.guaList_save = new List<int>{0,0,0,0,0,0,0,0};
        ...
        waveManager.waveNum = saveData.waveNum_save;
