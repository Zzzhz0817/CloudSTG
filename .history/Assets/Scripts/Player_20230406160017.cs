        ...
        if (!isPaused)
        {
            if (Input.touchCount == 1)
            {
                ...
                if (!xunOneFingerFlag)
                {
                    ...
                    var xunWindEffect = Instantiate(xunEffect, (transform.position + xunPosBuffer)/2f, Quaternion.Euler((transform.position - xunPosBuffer).x, (transform.position - xunPosBuffer).y, (transform.position - xunPosBuffer).z));
