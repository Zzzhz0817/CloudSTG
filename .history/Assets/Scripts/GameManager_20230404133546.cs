...
public class GameManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (waveTimer > waveInterval) 
        {
            ...
            if (waveNum % 5 == 1)
            {
                Spawn2(enemyShooter, new Vector2(1.5f, 5.5f));
                Spawn2(enemyShooter, new Vector2(-1.5f, 5.5f), 3000);
            }
            else if (waveNum % 5 == 2)
            {
    ...
    private async void Spawn2(GameObject enemy, Vector2 position, int delayTime = 0)
    {
        await Task.Run(() =>
        {
