...
public class Player : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (guaDict["Zhen"] > 0)
        {
            Debug.Log(guaDict["Zhen"]);
            Debug.Log("Math" + 30f * (2/(1+5)));
            zhenCooldownTime = 30f * (2/(1+guaDict["Zhen"]));
            Debug.Log(zhenCooldownTime);
        }
    ...
    void Update()
    {
        ...
        if (zhenActiveFlag && Input.touchCount == 1)
        {
            ...
            zhenEffect.transform.localScale = new Vector3(zhenEffect.transform.localScale.x *(transform.position - new Vector3(world_pos.x, world_pos.y, 0f)).magnitude, zhenEffect.transform.localScale.y * 0.25f, zhenEffect.transform.localScale.z);
        }
