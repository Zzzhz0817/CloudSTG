...
public class Player : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (guaDict["Li"] > 0)
        {
            ...
            currentProjectile.SetLevel(guaDict["Li"]);
        }
        if (guaDict["Kan"] > 0)
        {
            currentProjectile = waterProjectile;
            currentProjectile.SetLevel(guaDict["Kan"]);
        }
        if (guaDict["Li"] > 0 && guaDict["Kan"] > 0)
        {
            currentProjectile = utimateProjectile;
            currentProjectile.SetLevel(guaDict["Li"], guaDict["Kan"]);
        }
    }
    ...
    private async void Launch()
    {
        ...
        for (int i = 0; i <= guaDict["Qian"]; i++)
        {
            for (int j = 0; j <= guaDict["Kun"]; j++)
            {
                ...
                GameObject projectileObject1 = Instantiate(currentProjectile, transform.position, Quaternion.Euler(0f, 0f, orientationAngle-90-deltaAngle));
                ...
                if (j > 0)
                {
                    GameObject projectileObject2 = Instantiate(currentProjectile, transform.position, Quaternion.Euler(0f, 0f, orientationAngle-90+deltaAngle));
