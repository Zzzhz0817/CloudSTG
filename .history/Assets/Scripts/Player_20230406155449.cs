...
public class Player : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!isPaused)
        {
            if (Input.touchCount == 1)
            {
                ...
                if (!xunOneFingerFlag)
                {
                    Debug.Log(Quaternion.Euler(transform.position - xunPosBuffer));
                    Debug.Log((transform.position - xunPosBuffer).magnitude);
                    xunOneFingerFlag = true;
                    var xunWindEffect = Instantiate(xunEffect, (transform.position + xunPosBuffer)/2f, Quaternion.Euler(transform.position - xunPosBuffer +90));
            ...
            {   
                ...
                if (xunOneFingerFlag)
                {
                    ...
                    var xunWindEffect = Instantiate(xunEffect, (transform.position + xunPosBuffer)/2f, Quaternion.Euler(transform.position - xunPosBuffer));
                    xunWindEffect.transform.localScale = new Vector3(xunWindEffect.transform.localScale.x *(transform.position - xunPosBuffer).magnitude, xunWindEffect.transform.localScale.y, xunWindEffect.transform.localScale.z);
                }
                xunPosBuffer = transform.position;
            }
    ...
    public void GetYao(bool isYang)
    {
        ...
        if (yaoList.Count == 1)
        {
            spriteRenderer.sprite = wings[8+(yaoList[0] ? 1 : 0)];
        }
        else if (yaoList.Count == 2)
        {
            spriteRenderer.sprite = wings[10+(yaoList[0] ? 1 : 0)*2+(yaoList[1] ? 1 : 0)];
        }
        ...
        if (yaoList.Count >= 3)
        {
            spriteRenderer.sprite = wings[(yaoList[0] ? 1 : 0)*4 + (yaoList[1] ? 1 : 0)*2 + (yaoList[2] ? 1 : 0)];
